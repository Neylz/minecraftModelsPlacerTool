...
execute unless block ~ ~ ~ #mdlt:raycastthrough align xyz positioned ~.5 ~1 ~.5 as @e[type=item_display,tag=mdlt_model] if score @s mdlt.model.id = @a[tag=temp_player,limit=1] mdlt.model.id run function mdlt:private/editor/move/common/move
