...
execute unless block ~ ~ ~ #mdlt:raycastthrough align y positioned ~ ~1 ~ as @e[type=item_display,tag=mdlt_model,distance=..7,sort=nearest,limit=1] run function mdlt:private/editor/select/selected
execute if score #temp mdlt.data matches 0.. as @e[type=item_display,tag=mdlt_model,distance=...5,sort=nearest,limit=1] run function mdlt:private/editor/select/selected
